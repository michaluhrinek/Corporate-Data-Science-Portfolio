...
file_path = r"C:\Users\Downloads\synthetic_cybersecurity_data_with_days.csv"
...
# Step 2: Preprocessing text manually (without requiring NLTK tokenizer)
def preprocess_text(text):
    ...
    tokens = text.split()  # Tokenize using simple whitespace split
    filtered_words = [word for word in tokens if word not in CUSTOM_STOPWORDS and len(word) > 2]  # Remove stopwords and short words
    return " ".join(filtered_words)  # Return preprocessed text as a string


# Preprocess the entire dataset
preprocessed_descriptions = [preprocess_text(desc) for desc in short_descriptions]
...
# Step 4: Use CountVectorizer for bigram extraction
vectorizer = CountVectorizer(ngram_range=(2, 2))  # Extract bigrams without using built-in stopwords
