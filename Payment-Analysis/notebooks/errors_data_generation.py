...
process_system_map = {
    ...
    "Money Rates":            [("SOLEMOS",   1.00)],
    "Proces X":               [("SOLEMOS",   0.80), ("STP CAT", 0.20)],
}
...
short_descriptions = {
    ...
    "Proces X": [
        "Process X item approved without verifying beneficiary details",
        "Wrong payment details amended in Process X - error compounded",
        "Process X item released without required dual authorisation",
        "Incorrect amount corrected to wrong value during manual review",
        "Payment in Process X actioned by unauthorised operator",
        "Process X item closed without resolution - payment lost in queue",
        "Wrong IBAN updated during manual correction in Process X",
        "Process X SLA breached - payment delayed beyond agreed window",
        "Operator amended wrong field in Process X - further error introduced",
        "Process X item duplicated after system timeout during save",
        "Payment released from Process X to wrong processing queue",
        "Manual override applied in Process X without supervisor sign-off",
    ],
}
...
priority_weights_by_process = {
    ...
    "Money Rates":            {"Critical": 0.35, "High": 0.45, "Medium": 0.15, "Low": 0.05},
    "Proces X":               {"Critical": 0.10, "High": 0.35, "Medium": 0.45, "Low": 0.10},
}
...
# ─── SAVE CSV ──────────────────────────────────────────────────────────────────
output_path = r"payments_errors_2025.csv"
