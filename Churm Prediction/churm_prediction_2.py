...
from sklearn.preprocessing import StandardScaler #transform features so they all have the same scale mean of 0 and 1
from sklearn.metrics import (
    roc_auc_score, recall_score, precision_score,
    f1_score, confusion_matrix, classification_report
)
from xgboost import XGBClassifier #upgrade for accuracy
...
monthly_revenue      = np.random.uniform(50, 800, n) #revenue per customer per month random 
tenure_days          = np.random.randint(30, 1825, n)
price_increase       = np.random.binomial(1, 0.2, n)

# Churn probability — higher when:  it's the recipe used to generate who churns, so the model has something real to learn from.
# login gap long, few features, usage dropping, contract expiring
#-3 very engaged, churm probability = 5%, 0(neutral) 50% churm, +3(very at risk) 95% churm probability
churn_score = (
    0.03 * days_since_login # longer gap since login → more likely to churn
  - 0.05 * features_used   # using more features → less likely to churn
  - 0.3  * usage_trend     # usage going up → less likely to churn
  + 0.15 * support_tickets  # more support complaints → more likely to churn
  - 0.002 * contract_days_left  # contract almost expired → more likely to churn
  + 0.4  * price_increase     # got a price increase → more likely to churn
  + np.random.normal(0, 0.3, n)  # random noise — real life is never perfectly predictable
)
...
#preparing the data about customers with days to churn, tenure days, churned...so later we can work with it 
df = pl.DataFrame({
...
# Churn by features used  spliting customer into groups based on how many product feature they use
#features generated was 1-19 random so that why the number of features 
feature_groups = (
    df
    .with_columns(
...
churned_revenue = (
    df.filter(pl.col("churned") == 1)["monthly_revenue"].sum() #sum of all revenue for churned =1
)
...
xgb = XGBClassifier(
    n_estimators=100, max_depth=4, #n_estimators-number of trees, xboost builds many decision trees one after another, each one correcting the previous ones mistakes. 
    random_state=42, eval_metric="logloss", #max_depth-how deep the tree can go
    verbosity=0
)
xgb.fit(X_train, y_train)

y_pred_xgb  = xgb.predict(X_test)  #testing probability of cust. churm on test data
y_proba_xgb = xgb.predict_proba(X_test)[:, 1]  #what is the probability of people going/canceling?

auc_xgb  = roc_auc_score(y_test, y_proba_xgb)#overall ability to rank customers by risk, separates churners from non-churners across all ossible tresholds
rec_xgb  = recall_score(y_test, y_pred_xgb) #dit we catch all the churners?
prec_xgb = precision_score(y_test, y_pred_xgb)# when we raise an alarm are we right? 
f1_xgb   = f1_score(y_test, y_pred_xgb) #balance between recall and precision 
...
# ── STEP 9: SHAP — WHY IS EACH CUSTOMER AT RISK? ────────────
print("\n--- STEP 9: SHAP — Why will each customer churn? ---")
print("SHAP tells us not just WHO churns, but WHY — per customer.")
import shap
...
print("\nGlobal feature importance (average impact on churn):")
...
for row in shap_importance.iter_rows():
    print(f"  {row[0]:<20} {row[1]:.4f}")

# ── Per customer: explain the top 5 highest-risk customers ───
print("\nPer-customer explanation — Top 5 highest-risk customers:")
print("(positive SHAP = pushes toward churn, negative = protects)")

top5_idx = np.argsort(y_proba_xgb)[-5:][::-1]

for rank, i in enumerate(top5_idx, 1):
    cust_id   = priority["customer_id"][rank - 1]
    cust_prob = y_proba_xgb[i]
    print(f"\n  #{rank} {cust_id} — churn probability: {cust_prob:.1%}")
    cust_shap = pl.DataFrame({
        "feature": features,
        "shap":    shap_values[i].tolist(),
        "value":   X_test[i].tolist()
    }).sort("shap", descending=True)
    for r in cust_shap.iter_rows():
        direction = "→ CHURN" if r[1] > 0 else "→ safe"
        print(f"    {r[0]:<20} value={r[2]:>8.1f}  shap={r[1]:>+.4f}  {direction}")
